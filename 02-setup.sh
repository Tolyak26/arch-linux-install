...
echo ""
pacman -S --noconfirm - < /root/arch-linux-install/pkg-lists/pkg-networkmanager.txt
echo ""

echo "- Installing additional packages for Arch Linux system ... "
echo ""
pacman -S --noconfirm - < /root/arch-linux-install/pkg-lists/pkg-arch-additional.txt
echo ""

echo "- Installing Xorg packages ... "
echo ""
pacman -S --noconfirm - < /root/arch-linux-install/pkg-lists/pkg-xorg.txt
echo ""
...
if grep -E "NVIDIA|GeForce" <<< ${get_gpu_vendor}; then
    ...
    pacman -S --noconfirm - < /root/arch-linux-install/pkg-lists/pkg-driver-video-nvidia.txt
	sed -i 's/^MODULES=()/MODULES=(nvidia)/' /etc/mkinitcpio.conf
	nvidia-xconfig
elif lspci | grep 'VGA' | grep -E "AMD"; then
	echo "Installing AMDGPU packages"
    pacman -S --noconfirm - < /root/arch-linux-install/pkg-lists/pkg-driver-video-amd.txt
	sed -i 's/^MODULES=()/MODULES=(amdgpu)/' /etc/mkinitcpio.conf
elif lspci | grep 'VGA' | grep -E "ATI"; then
	echo "Installing ATI Legacy packages"
    pacman -S --noconfirm - < /root/arch-linux-install/pkg-lists/pkg-driver-video-ati.txt
    ...
    pacman -S --noconfirm - < /root/arch-linux-install/pkg-lists/pkg-driver-video-intel.txt
